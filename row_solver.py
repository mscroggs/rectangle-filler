...
for i, j, n, extras in options:
    ...
    for n0 in range(large[0] // small[0] + 1):
        for n1 in range((large[0] - n0 * small[0]) // small[1] + 1):
            for n2 in range(
                (large[0] - n0 * small[0] - n1 * small[1]) // (small[0] + small[1]) + 1
            ):
    ...
    for x in range(largest[2]):
        ...
        for a in range(best_row[1]):
            rectangles.append(
                [
                    [large[1] - small[0] * (a + 1), o],
                    [large[1] - small[0] * a, o + small[1]],
                ]
            )
        for a in range(best_row[2]):
            rectangles.append(
                [
                    [large[1] - small[1] * (a + 1), o + small[1]],
                    [large[1] - small[1] * a, o + small[1] + small[0]],
                ]
            )
    ...
    print(
        f"Fitting {largest[3]} (greater than {2 ** (i - j)}) pieces of A{i} paper on a piece of A{j} paper"
    )
